...
<div class="container">
	<header class="issue__header">
		...
		{if $issue->getDatePublished()}
			<p class="issue__meta">{translate key="plugins.themes.pageone.issue.published"} {$issue->getDatePublished()|date_format:$dateFormatLong}</p>
		{/if}
		{/strip}
	</header>
	...
	{if $issue->getLocalizedDescription() || $issueGalleys}
		<section class="row issue-desc">
			...
			{if $issue->getLocalizedDescription()}
				<div class="col-md-6">
					<h3 class="issue-desc__title">{translate key="plugins.themes.pageone.issue.description"}</h3>
					<div class="issue-desc__content">
						...
						{else}
							...
							<a class="full-issue__link"
							   href="{url op="view" page="issue" path=$issue->getBestIssueId()}">{translate key="plugins.themes.pageone.issue.fullIssueLink"}</a>
						{/if}
					</div>
				</div>
			{/if}
