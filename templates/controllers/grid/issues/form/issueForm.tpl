...
<form class="pkp_form" id="issueForm" method="post" action="{url op="updateIssue" issueId=$issueId}">
	...
	{if !empty($sections)}
		{fbvFormArea id="sectionArea" title="editor.issues.sectionArea"}
			{fbvFormSection title="plugins.themes.pageone.colorPick" inline=false size=$fbvStyles.size.MEDIUM}
				{foreach from=$sections item=section}
					{assign var=sectionId value=$section->getId()}
					{* Color picker for issue's sections*}
					<label for="pageoneSectionColor-{$sectionId}">{$section->getLocalizedTitle()|escape}</label>
					<input
						id="pageoneSectionColor-{$sectionId}"
						type="color"
						name="pageoneSectionColor[{$sectionId}]"
						value="{$pageoneSectionColor[$sectionId]}"
					/>
				{/foreach}
			{/fbvFormSection}
		{/fbvFormArea}
